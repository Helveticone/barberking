...
async function handleApi(request, env, url) {
  ...
  if (path.startsWith("/admin/")) {
    ...
    if (path === "/admin/appointments" && method === "GET") {
      ...
      const rows = await env.DB.prepare(
        `SELECT a.id, a.client_name, a.client_phone, a.start_at, a.end_at, a.status,
                s.name AS service_name, s.price_chf
    ...
    if (path === "/admin/blocks" && method === "GET") {
      ...
      const to = url.searchParams.get("to");
      const rows = await env.DB.prepare(
        "SELECT id, start_at, end_at, reason FROM blocks WHERE start_at < ? AND end_at > ? ORDER BY start_at"
      ).bind(to, from).all();
      return json(rows.results || []);
    }
